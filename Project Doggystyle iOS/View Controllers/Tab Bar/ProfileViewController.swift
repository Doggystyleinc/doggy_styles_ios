...
//        self.nameLabel.text = "\(userProfileStruct.user_first_name ?? "Pet") \(userProfileStruct.user_last_name ?? "Lover")"
//        
//        //Set profile image with animation
//        profileImageView.sd_setImage(with: URL(string: userProfileStruct.profile_image_url ?? ""), placeholderImage: UIImage(named: Constants.ownerProfilePlaceholder)) { _, _, _, _ in
